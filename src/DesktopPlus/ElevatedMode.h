...
//There's no way to actually trust the unelevated process requesting the inputs. Even if we isolated DesktopPlus.exe, we still couldn't trust SteamVR to not be comprimised.
...
//Targetted attacks would be fairly easy if such code found the way of an user's machine, though.
